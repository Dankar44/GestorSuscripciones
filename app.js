...
let items=[];try{const cached=JSON.parse(localStorage.getItem(KEY));if(Array.isArray(cached))items=cached}catch{}
...
const cacheLocal=()=>localStorage.setItem(KEY,JSON.stringify(items));
async function dbUpsert(item){try{await window.db.upsert(item);cacheLocal()}catch(e){toast('Error al guardar: '+(e.message||e));throw e}}
async function dbDelete(id){try{await window.db.remove(id);cacheLocal()}catch(e){toast('Error al eliminar: '+(e.message||e));throw e}}
async function loadFromDb(){try{items=await window.db.list();cacheLocal();render()}catch(e){toast('No se pudieron cargar tus datos: '+(e.message||e))}}
window.__onAuthUser=async user=>{if(user){await loadFromDb()}else{items=[];render()}};
...
$('#subscriptionForm').onsubmit=async e=>{e.preventDefault();const d=new FormData(e.target),id=d.get('editingId'),entry={id:id||uid(),name:String(d.get('name')).trim(),price:Number(d.get('price')),currency:d.get('currency'),renewalDate:d.get('renewalDate'),frequency:d.get('frequency'),category:d.get('category'),cancelBeforeRenewal:d.get('cancelBeforeRenewal')==='on'};if(pendingIcon)entry.logo=pendingIcon;else if(id){const old=items.find(x=>x.id===id);if(old?.logo)entry.logo=old.logo}const prev=items.slice();if(id)items=items.map(x=>x.id===id?entry:x);else items.push(entry);try{await dbUpsert(entry);render();modal(false);toast(id?'Cambios guardados':'Suscripción guardada')}catch{items=prev;render()}};
document.addEventListener('click',async e=>{const view=e.target.closest('[data-view]')?.dataset.view||e.target.closest('[data-view-target]')?.dataset.viewTarget;if(view){switchView(view);return}const day=e.target.closest('[data-date]');if(day){selectedDate=day.dataset.date;renderCalendar();return}const edit=e.target.closest('[data-edit]');if(edit){openEdit(edit.dataset.edit);return}const icon=e.target.closest('[data-icon]');if(icon){chooseExistingIcon(icon.dataset.icon);return}const del=e.target.closest('[data-delete]');if(del){const id=del.dataset.delete,prev=items.slice();items=items.filter(x=>x.id!==id);try{await dbDelete(id);render();toast('Suscripción eliminada')}catch{items=prev;render()}return}const tog=e.target.closest('[data-toggle]');if(tog){const x=items.find(i=>i.id===tog.dataset.toggle);if(x){x.cancelBeforeRenewal=!x.cancelBeforeRenewal;try{await dbUpsert(x);render();toast('Estado actualizado')}catch{x.cancelBeforeRenewal=!x.cancelBeforeRenewal;render()}}}});
